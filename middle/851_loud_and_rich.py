# 在一组 N 个人（编号为0, 1, 2, ..., N-1）中，
# 每个人都有不同数目的钱，以及不同程度的安静（quietness）。
# 
# 为了方便起见，我们将编号为x的人简称为 "personx"。
# 
# 如果能够肯定 personx比 persony更有钱的话，
# 我们会说richer[i] = [x, y]。注意richer可能只是有效观察的一个子集。
# 
# 另外，如果 personx的安静程度为q，我们会说quiet[x] = q。
# 
# 现在，返回答案answer，其中answer[x] = y的前提是，
# 在所有拥有的钱不少于personx的人中，persony是最安静的人（也就是安静值quiet[y]最小的人）。
# 
# 来源：力扣（LeetCode）
        ...
        def find(index):
            ...
            if result[index] == -1:
                for i in range(len(d[index])):
                    if quiet[d[index][i]] < quiet[small]:
                        small = d[index][i]
                    a = find(d[index][i])
                    if quiet[small] > quiet[a]:
                        small = a
                result[index] = small
            return result[index]
